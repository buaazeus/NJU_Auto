...
class CategoricalPd(Pd):
    ...
    def neglogp(self, x):
        ...
        if x.dtype in {tf.uint8, tf.int32, tf.int64, tf.uint32}:
            # one-hot encoding
...
class MultiCategoricalPd(Pd):
    def __init__(self, nvec, flat):
        ...
        self.flat = flat
        # nenv = [31,12]
        nenv = np.array([31,12],dtype=np.int32)
...
class DiagGaussianPd(Pd):
    ...
    def entropy(self):
        return tf.reduce_sum(self.logstd + .5 * np.log(2.0 * np.pi * np.e), axis=-1)  #.5 * np.log(2.0 * np.pi * np.e) 为 1.4186
    def sample(self):
